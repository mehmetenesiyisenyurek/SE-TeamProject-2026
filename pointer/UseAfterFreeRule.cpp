...
std::string UseAfterFreeRule::getId() const { return "R008"; }
std::string UseAfterFreeRule::getName() const { return "Use After Free Rule"; }
DiagnosticSeverity UseAfterFreeRule::getDefaultSeverity() const { return DiagnosticSeverity::CRITICAL; }

std::vector<Diagnostic> UseAfterFreeRule::check(const ASTNode& ast) {
    std::vector<Diagnostic> diagnostics;

    if (ast.getType() == ASTNodeType::FUNCTION_DEF) {
        auto freeCalls = findFreeCallSites(ast);
...
std::vector<std::pair<std::string, int>> UseAfterFreeRule::findFreeCallSites(const ASTNode& node) const {
    ...
    for (const ASTNode* child : node.getChildren()) {
        if (child != nullptr) {
            auto childResults = findFreeCallSites(*child);
...
bool UseAfterFreeRule::isUsedAfterFree(const std::string& varName, int freeLine, const ASTNode& scope) const {
    if (scope.getLine() > freeLine) {
        if (isNullAssignment(varName, scope)) return false;
        if (scope.getValue() == varName) return true;
    }

    for (const ASTNode* child : scope.getChildren()) {
        if (child != nullptr && isUsedAfterFree(varName, freeLine, *child)) return true;
    }
